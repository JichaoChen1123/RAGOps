...
export function EvaluationsPage() {
  ...
  const averageQualityScore = evaluatedScores.length > 0
    ...
    : '未知';
  const activeDatasetSampleCount = availableDatasets.find((item) => item.id === draft.datasetId)?.sampleCount ?? 0;
  const sampleTotal = datasetSamples.length > 0 ? datasetSamples.length : activeDatasetSampleCount;
  // An explicit partial selection can be empty. Never use a truthy fallback here:
  // doing so would report metadata's total after the user deliberately cleared it.
  const selectedSampleCount = selectedSampleIds === null ? sampleTotal : selectedSampleIds.length;
    ...
    <>
      ...
      >
        <form className="form-grid" id="create-evaluation-form" onSubmit={createTask}>
          ...
          <fieldset className="field-full sample-selection"><legend>评测样本（{selectedSampleCount} / {sampleTotal}）</legend><div className="sample-mode" role="radiogroup" aria-label="样本范围"><label><input type="radio" name="sample-mode" checked={selectedSampleIds === null} onChange={() => setSelectedSampleIds(null)} /> <span>全部样本</span></label><label><input type="radio" name="sample-mode" checked={selectedSampleIds !== null} onChange={() => setSelectedSampleIds(datasetSamples.map((item) => item.sampleId))} /> <span>选择部分</span></label></div>{selectedSampleIds !== null && <><div className="sample-picker-actions"><span aria-live="polite">已选 {selectedSampleIds.length} / {sampleTotal}</span><button type="button" onClick={() => setSelectedSampleIds(datasetSamples.map((item) => item.sampleId))}>全选</button><button type="button" onClick={() => setSelectedSampleIds([])}>清空</button></div><div className="sample-picker" role="group" aria-label="可选评测样本">{datasetSamples.map((sample) => <label key={sample.sampleId} className="sample-picker-row"><input type="checkbox" checked={selectedSampleIds.includes(sample.sampleId)} onChange={() => setSelectedSampleIds((current) => current?.includes(sample.sampleId) ? current.filter((id) => id !== sample.sampleId) : [...(current ?? []), sample.sampleId])} /><span>{sample.question}</span></label>)}</div></>}{selectedSampleIds !== null && selectedSampleIds.length === 0 && <p className="form-hint">请至少选择一条样本；空选择不会提交。</p>}</fieldset>
