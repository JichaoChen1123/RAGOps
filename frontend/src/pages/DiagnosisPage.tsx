    ...
    <>
      ...
      <div className="evidence-layout">
        ...
        <section className="answer-column">
          ...
          {diagnosis.run.error && <div className="run-error" role="alert"><strong>{diagnosis.run.error.code}{diagnosis.run.error.reasonCode ? ` · ${diagnosis.run.error.reasonCode}` : ''}</strong><p>{diagnosis.run.error.message}</p><small>{diagnosis.run.error.diagnosticId ? `诊断 ID ${diagnosis.run.error.diagnosticId} · ` : ''}尝试 {diagnosis.run.error.attempts} 次 · {diagnosis.run.error.retryable ? '可重试' : '不可自动重试'}</small></div>}
