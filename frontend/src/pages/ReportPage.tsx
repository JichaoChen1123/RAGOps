...
const generationLabel = (value: Record<string, unknown> | undefined) => value
  ...
  return (
    <>
      ...
      <div className="summary-strip report-status-strip" aria-label="运行与质量状态">
        ...
        <div><span>质量状态</span><strong><StatusBadge value={report.qualitySummary.status} /></strong><small>{qualityCoverage}</small></div>
      ...
      <div className="dashboard-grid report-grid">
        ...
        <Panel title="运行快照" eyebrow="REPRODUCIBILITY">
          <dl className="config-list">
            <div><dt>执行器 / 提供方</dt><dd><code>{snapshot?.adapterId ?? report.task.adapterId ?? '未知'}</code> / <code>{snapshot?.providerName ?? snapshot?.providerId ?? report.task.providerId ?? '无或未知'}</code></dd></div>
            <div><dt>请求模型</dt><dd><code>{snapshot?.generation.model ?? report.task.modelVersion ?? '未知'}</code></dd></div>
            <div><dt>实际生效参数</dt><dd><code>{generationLabel(snapshot?.effectiveGeneration)}</code></dd></div>
          ...
          <table>
            ...
            <tbody>{filteredSamples.map((sample) => (
              <tr key={sample.id}>
                ...
                <td><StatusBadge value={sample.qualityStatus} /><small>{sample.metricState === 'metrics_calculated' ? '指标已计算' : '指标未计算'} · {sample.qualityGateState === 'quality_gate_evaluated' ? '质量门已评估' : '质量门未配置'}</small></td>
