...
export function DatasetsPage() {
  ...
  const ensureMockWrite = () => {
    if (apiMode === 'mock') return true;
    setFeedback('LIVE API 模式当前只读，请通过后端数据集接口执行写入');
    return false;
  };

  const importExample = () => {
    if (!ensureMockWrite()) return;
    const imported: DisplayDataset = {
      id: `ds-mock-import-${Date.now()}`,
      name: '退款政策示例 JSONL',
      description: '[Mock 导入] 12 条退款政策问答样本，仅保存在当前页面会话。',
      sampleCount: 12,
      status: 'ready',
      version: 'v0.1',
      coverage: 72,
    ...
    setLocalDatasets((current) => [imported, ...(current ?? [])]);
    setDialog(null);
    setFeedback('Mock 示例 JSONL 导入完成，已新增 12 条样本');
  };

  const createDataset = (event: FormEvent) => {
    event.preventDefault();
    if (!ensureMockWrite()) return;
    const created: DisplayDataset = {
      id: `ds-mock-${Date.now()}`,
      name: draft.name.trim(),
      description: draft.description.trim() || '[Mock 新建] 待补充数据集说明。',
      sampleCount: 0,
      status: 'draft',
      version: 'v0.1',
      coverage: 0,
    ...
    setLocalDatasets((current) => [created, ...(current ?? [])]);
    setDraft({ name: '', description: '', owner: '当前用户' });
    setDialog(null);
    setFeedback(`已在前端状态中新建 Mock 数据集“${created.name}”`);
  };
  ...
  return (
    <>
      ...
      <Dialog
        ...
        footer={<><button className="button button-secondary" type="button" onClick={() => setDialog(null)}>取消</button><button className="button button-primary" type="button" onClick={importExample} disabled={apiMode !== 'mock'}><Upload size={15} />导入示例 JSONL</button></>}
      >
        ...
        <p className="form-hint">{apiMode === 'mock' ? '导入结果会明确标记为 MOCK，并只保存在当前页面会话。' : 'LIVE API 模式当前只读；请通过后端数据集导入接口写入。'}</p>
      </Dialog>
      ...
      <Dialog
        ...
        eyebrow="LOCAL DRAFT"
        onClose={() => setDialog(null)}
        footer={<><button className="button button-secondary" type="button" onClick={() => setDialog(null)}>取消</button><button className="button button-primary" type="submit" form="create-dataset-form" disabled={apiMode !== 'mock'}><FilePlus2 size={15} />创建 Mock 数据集</button></>}
      >
        ...
        <p className="form-hint">{apiMode === 'mock' ? '将创建一个样本量为 0 的 Mock 草稿，不会写入真实服务。' : 'LIVE API 模式当前只读。'}</p>
      </Dialog>
